...
## Acceptance Criteria
- [ ] `Bingo.slnx` exists at repo root.
- [ ] No `.sln` file is present in the repo.
- [ ] All existing projects are included in `Bingo.slnx`.
- [ ] `dotnet build Bingo.slnx` succeeds on a clean machine.

## Implementation Tasks
- [ ] Create folder layout (if not already present):
  - `src/Bingo.Api`
  - `src/Bingo.Web`
  - `src/Bingo.Core`
  - `src/Bingo.Data`
  - `src/Bingo.AppHost`
  - `tests/Bingo.*.Tests`
- [ ] Create `Bingo.slnx` at repo root.
- [ ] Add all projects to the solution.
- [ ] Add a guardrail (docs or CI) ensuring `.sln` files aren’t committed.
