...
- [x] `src/Bingo.AppHost` exists and builds.
- [x] The distributed app can be started using a single Aspire CLI command.
- [x] README contains only minimal Aspire guidance:
  - install Aspire CLI
  - run the single command to start locally
- [x] No hand-written docker compose files are added.

## Implementation Tasks

- [x] Determine required Aspire CLI version (or compatible range) for .NET 10.
- [x] Scaffold AppHost via Aspire CLI.
- [x] Add/verify Aspire service defaults integration.
- [x] Verify local run starts the Aspire dashboard.
